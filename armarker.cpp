...
ARPattHandle *ARMarker::ARPatternHandle = nullptr;

ARMarker::ARMarker(FbxManager *sdkManager,const char* modelFile, const char *pattName, ARMarker::ARMarkerType type)
{
    if(! ARMarker::ARPatternHandle)
    {
        if (!(ARMarker::ARPatternHandle = arPattCreateHandle()))
        {
    ...
    if (sdkManager)
    {
        ...
        m_fbxScene = FbxScene::Create(sdkManager, "My Scene");
        if( !m_fbxScene )
        {
        ...
        // Create the importer.
        int lFileFormat = -1;
        mImporter = FbxImporter::Create(sdkManager,"");
        if (!sdkManager->GetIOPluginRegistry()->DetectReaderFileFormat(modelFile, lFileFormat) )
        {
            // Unrecognizable file format. Try to fall back to FbxImporter::eFBX_BINARY
            lFileFormat = sdkManager->GetIOPluginRegistry()->FindReaderIDByDescription( "Binary fbx (*.fbx)" );;
        }

        // Initialize the importer by providing a filename.
        if(mImporter->Initialize(modelFile, lFileFormat))
        {
            qDebug() << "init importer";
        }
    }
    else
    {
       qDebug() <<"Unable to create the FBX SDK manager";
    }
    ...
    if ((m_patternId = arPattLoad(ARPatternHandle, pattName)) < 0)
    {
        ARLOGe("ARMarker: Error loading pattern file %s.\n", pattName);
        return ;
    }
}
...
ARMarker::~ARMarker()
{
    if(ARPatternHandle) arPattDeleteHandle(ARPatternHandle);
    ARPatternHandle = nullptr;
}

ARdouble ARMarker::makeTransformationMatrix(ARMarkerInfo *markerInfo, AR3DHandle *gAR3DHandle)
{
    return arGetTransMatSquare(gAR3DHandle, markerInfo, gPatt_width, gPatt_trans);
}
...
ARMarker::ARMarkerType ARMarker::type() const
{
    return m_type;
}

bool ARMarker::LoadFile()
{
    bool lResult = false;
    // Make sure that the scene is ready to load.

    if (mImporter->Import(m_fbxScene))
    {
        ...
        lGeomConverter.Triangulate(m_fbxScene, /*replace*/true);

        lResult = true;
    }
    else
    {
        qDebug() << mImporter->GetStatus().GetCode() << "Unable to import file ";
        mImporter->Destroy();
        mImporter = nullptr;
    }
...
void ARMarker::checkNodeRecursive(FbxNode *pNode)
{
    ...
    {
        checkNodeRecursive(pNode->GetChild(lChildIndex));
    }
}

void ARMarker::checkNode(FbxNode *pNode)
{

}

void ARMarker::drawLightSource() const
{
    glPushMatrix(); // Save world coordinate system.
    glScalef(m_modelSize,m_modelSize,m_modelSize);
    ...
    if (mPoseIndex != -1)
    {
        lPose = m_fbxScene->GetPose(mPoseIndex);
    }
    ...
    m_fbxScene->FillAnimStackNameArray(mAnimStackNameArray);
    FbxAnimStack * lCurrentAnimationStack = nullptr;

    if(mAnimStackNameArray.Size() > 0)
    {
            lCurrentAnimationStack = m_fbxScene->FindMember<FbxAnimStack>(mAnimStackNameArray[0]->Buffer());
    }

    FbxTime start = lTimeLineTimeSpan.GetStart();

    InitializeLights(m_fbxScene, start, lPose);

    if(!lCurrentAnimationStack)
    {
        DrawNodeRecursive(m_fbxScene->GetRootNode(),start , nullptr, lDummyGlobalPosition, lPose, ShadingMode::SHADING_MODE_SHADED);
    }
    else
    {
        DrawNodeRecursive(m_fbxScene->GetRootNode(),start , lCurrentAnimationStack->GetMember<FbxAnimLayer>(), lDummyGlobalPosition, lPose, ShadingMode::SHADING_MODE_SHADED);
    }
...
void ARMarker::drawObject() const
{    

}

void ARMarker::setModelSize(const float &modelSize)
{
    m_modelSize = modelSize;
}

int ARMarker::found() const
{
    return m_found;
}

void ARMarker::setFound(int found)
{
    m_found = found;
}

float ARMarker::modelSize()
{
    return m_modelSize;
}

int ARMarker::id() const
{
    return m_patternId;
}

void ARMarker::InitializeLights(FbxScene* pScene, const FbxTime & pTime, FbxPose* pPose) const
{
...
void ARMarker::DrawNodeRecursive(FbxNode* pNode, FbxTime& pTime, FbxAnimLayer* pAnimLayer,
...
void ARMarker::DrawNode(FbxNode* pNode,
              FbxTime& pTime,
              FbxAnimLayer* pAnimLayer,
              FbxAMatrix& pParentGlobalPosition,
              FbxAMatrix& pGlobalPosition,
              FbxPose* pPose, ShadingMode pShadingMode) const
{
...
void ARMarker::DrawMarker(FbxAMatrix& pGlobalPosition) const
{
...
void ARMarker::DrawSkeleton(FbxNode* pNode, FbxAMatrix& pParentGlobalPosition, FbxAMatrix& pGlobalPosition) const
{
...
void ARMarker::DrawMesh(FbxNode* pNode, FbxTime& pTime, FbxAnimLayer* pAnimLayer,
              FbxAMatrix& pGlobalPosition, FbxPose* pPose, ShadingMode pShadingMode) const
{
    ...
    if(m_type != ARMarkerType::LIGHTSOURCE)
    {
        if(lMesh->BBoxMax.Get().mData[0] == 0) lMesh->ComputeBBox();

        glColor4f(1.0f, 0.2f, 0.7f, 1.0f);
        glutWireCube(lMesh->BBoxMax.Get().mData[0] * m_modelSize);
        emit meshForCheck(lMesh , lVertexArray, pGlobalPosition);
    }
...
void ARMarker::ComputeShapeDeformation(FbxMesh* pMesh, FbxTime& pTime, FbxAnimLayer * pAnimLayer, FbxVector4* pVertexArray) const
{
...
void ARMarker::ComputeClusterDeformation(FbxAMatrix& pGlobalPosition,
                               FbxMesh* pMesh,
                               FbxCluster* pCluster,
...
void ARMarker::ComputeLinearDeformation(FbxAMatrix& pGlobalPosition,
                               FbxMesh* pMesh,
                               FbxTime& pTime,
...
void ARMarker::ComputeDualQuaternionDeformation(FbxAMatrix& pGlobalPosition,
                                     FbxMesh* pMesh,
                                     FbxTime& pTime,
                                     FbxVector4* pVertexArray,
                                     FbxPose* pPose) const
...
void ARMarker::ComputeSkinDeformation(FbxAMatrix& pGlobalPosition,
                                     FbxMesh* pMesh,
                                     FbxTime& pTime,
                                     FbxVector4* pVertexArray,
                                     FbxPose* pPose) const
...
void ARMarker::ReadVertexCacheData(FbxMesh* pMesh,
                         FbxTime& pTime,
...
void ARMarker::DrawCamera(FbxNode* pNode,
                FbxTime& pTime,
                FbxAnimLayer* pAnimLayer,
...
void ARMarker::DrawLight(FbxNode* pNode, const FbxTime& pTime, const FbxAMatrix& pGlobalPosition) const
{
...
void ARMarker::DrawNull(FbxAMatrix& pGlobalPosition) const
{
...
void ARMarker::MatrixScale(FbxAMatrix& pMatrix, double pValue) const
{
    int i,j;
...
void ARMarker::MatrixAddToDiagonal(FbxAMatrix& pMatrix, double pValue) const
{
    pMatrix[0][0] += pValue;
    pMatrix[1][1] += pValue;
    pMatrix[2][2] += pValue;
...
void ARMarker::MatrixAdd(FbxAMatrix& pDstMatrix, FbxAMatrix& pSrcMatrix) const
{
    int i,j;
